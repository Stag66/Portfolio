#Author: Jeremiah Airhart
#Modified by: Philip Wingfield
#Modified by: Drew Billy
#Modified by: Kareem Gordon-Graham
...
$PITTITVIRLABNUM = 0;
$REMOTEWORKNUM = 0;
$REMOTEWORKWINDOWS11NUM = 0;
$PITTITWVDNUM = 0;
$PITTITHIPAAFZNUM = 0;
$SSOEVIRLABNUM = 0;

$AZWVDModule = Get-Module -Name Az.DesktopVirtualization
$AZAZResourcesModule = Get-Module -Name Az.Resources
...
$Tests = 'BAS-AVD-POOL','PittIT-Student-Virtual-Computing-Lab','REmote-Work-Pool','Remote-Work-Pool-WIndows11','PITTIT_WVD','PITTIT-HIPAA-FZ','SSOE-Virtual-Lab-2021-2022'

#Module installation if needed
If ($AZWVDModule -eq $null) {
...
while($true) 
{
    ...
    if($FirstPass -or ($currTime -eq "00:00") -or ($currTime -eq "30:00"))
    {
        ...
        Switch ($Currenthost)
        {
            BAS-AVD-POOL{$BASAVDPOOLNUM = $AvailHosts }
            PittIT-Student-Virtual-Computing-Lab{$PITTITVIRLABNUM = $AvailHosts}
            REmote-Work-Pool{$REMOTEWORKNUM = $AvailHosts}
            Remote-Work-Pool-WIndows11{$REMOTEWORKWINDOWS11NUM = $AvailHosts}
            PITTIT_WVD{$PITTITWVDNUM = $AvailHosts}
            PITTIT-HIPAA-FZ{$PITTITHIPAAFZNUM = $AvailHosts}
            SSOE-Virtual-Lab-2021-2022{$SSOEVIRLABNUM = $AvailHosts}
        }
        ...
        Write-host "Total Hosts Available in Hostpool "SSOE-Virtual-Lab-2021-2022" = "$SSOEVIRLABNUM -ForegroundColor Yellow
        Get-date 
